    ...
    <div className="bg-[#2E3192] flex flex-col">
      <div className="wrapper flex justify-between ">
        <div className="flex flex-col gap-2">
          ...
          <p className="max-w-xs text-[#D1D5DB] text-sm">
        ...
        <div className="flex flex-col gap-2">
          <div className="font-semibold text-[#F8FAFC] text-2xl">
            Contact Us
          </div>
          {footer[0].contact.map((item, index) => (
            <div
              key={index}
              className="max-w-40 text-[#D1D5DB] text-sm flex flex-col gap-2"
            >
              <Link
                href={item.url}
        ...
        <div className="flex flex-col gap-2">
          ...
          {nav[0].navMenu?.map((item, index) => (
            <div
              key={index}
              className="text-[#D1D5DB] text-sm flex flex-col gap-2"
            >
              <Link
                href={item.path}
                className="hover:text-white hover:underline"
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="text-[#D1D5DB] text-sm flex gap-6">
            {nav[1].navSocial?.map((item, index) => (
              <Link
                key={index}
                href={item.path}
                className="relative h-[45px] w-[45px] group hover:scale-95 transition-transform duration-300 ease-in-out"
              >
                <Image
                  src={item.img}
                  alt={item.name}
